...
char *ip_num = "192.168.1.169";
module_param(ip_num, charp, S_IRUGO);
MODULE_PARM_DESC(ip_num, "ip address of usbq");
EXPORT_SYMBOL(ip_num);
/*-------------------------------------------------------------------------*/
...
ubq_driver_init(void)
{
   ...
   in4_pton(ip_num,strlen(ip_num),(u8*)&options.addr,'\0',NULL);
   options.connect = 1;
