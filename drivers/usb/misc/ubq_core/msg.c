...
char *dump_msg(const msg_t *m)
{
   if (!m) {
      ...
      snprintf(debug_msg,256,"msg:%p asize:%lu data_size:%lu %s",m,m->allocated_size,msg_get_data_size(m),dump_endpoint_id(&m->epid));
   } else if (IS_USB_ACK(m)) {
      snprintf(debug_msg,256,"msg:%p asize:%lu data_size:%lu ACK status:%d %s",m,m->allocated_size,msg_get_data_size(m),m->status,dump_endpoint_id(&m->epid));
   } else {
      snprintf(debug_msg,256,"msg:%p asize:%lu data_size:%lu type:%u management_type:%u",m,m->allocated_size,msg_get_data_size(m),m->type,m->management_type);
   }
   return debug_msg;
}
